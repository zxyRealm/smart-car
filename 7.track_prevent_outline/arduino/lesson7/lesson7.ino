...
void loop() 
{
  ...
  {
      //黑线循迹时速度要慢，否则很容易出线
   analogWrite(left_motor_en,110);//左电机占空比值 取值范围0-255，255最快
   analogWrite(right_motor_en,110);//右电机占空比值取值范围0-255 ,255最快 
  ...
  }else if(digitalRead(left_led1) == HIGH && digitalRead(right_led1) == LOW)
  {
       ...
       left();//右边出线，执行左转修正
    }else if(digitalRead(left_led1) == LOW && digitalRead(right_led1) == LOW)
    {
